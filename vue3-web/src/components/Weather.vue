<template>
  <div class="hello">
    ...
    <div  class="border-2 border-solid border-b-blue-600">
      component v-model
      <PetName class="placeholder:italic placeholder:text-slate-400 block bg-white w-full border border-slate-300 rounded-md py-2 pl-9 pr-3 shadow-sm focus:outline-none focus:border-sky-500 focus:ring-sky-500 focus:ring-1 sm:text-sm w-96" v-model="pet"></PetName>
    </div>
  </div>
</template>
...
let pet=$ref('pet')
 ...
 let   yesterday=$ref(null)


   function greet() {
      FetchApi
        .get(`/bidianer/api/weather/get-info?city_name=${input2}&simple=true`)
        .then((res) => {
          console.log(res)
         city = res.data .city;
         temperature = res.data.data.wendu;
          let foresee = res.data.data.forecast;
