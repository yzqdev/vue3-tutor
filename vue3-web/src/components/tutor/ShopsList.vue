  ...
  <div class="page-shopping-cart" id="shopping-cart">
    ...
    <div class="cart-product-title clearfix">
      <div class="td-check fl">
        <span class="check-span fl check-all" :class="{ 'check-true': isSelectAll }" @click="selectProduct(isSelectAll)"></span>
        全选
      </div>
...
import img1 from "../../assets/庄周.jpg";
import luna from "../../assets/露娜.jpg";
import zhaojun from "../../assets/王昭君.jpg";
...
let isSelectAll = computed(() => {
  ...
  return productList.value.every((val) => {
    return val.select;
  });
});
//获取总价和产品总件数
let getTotal = computed(() => {
  ...
  let _proList = productList.value.filter(function (val) {
      return val.select;
    }),
    totalPrice = 0;
...
//全选与取消全选
function selectProduct(_isSelect) {
  //遍历productList，全部取反
  for (let i = 0, len = productList.value.length; i < len; i++) {
    productList.value[i].select = !_isSelect;
  }
}

//删除已经选中(select=true)的产品
function deleteProduct() {
  productList.value = productList.value.filter(function (item) {
    return !item.select;
  });
}
