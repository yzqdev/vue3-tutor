    ...
    <div class="list">
      ...
      <p><label for="all">
        <input
            ...
            @click="chooseList(1)"
        />所有目标
      </label> <label for="finish">
        已完成目标  <input id="finish" type="radio" name="chooseType" @click="chooseList(2)"/>
      </label><label for="unfinish">
        <input id="unfinish" type="radio" name="chooseType" @click="chooseList(3)"/>
        未完成目标 </label>
      </p>
    </div>
    <ul>
...
export default {
  ...
  methods: {
    ...
    delectList(list) {
      let index = this.prolist.indexOf(list);
      //根据索引，删除数组某一项
