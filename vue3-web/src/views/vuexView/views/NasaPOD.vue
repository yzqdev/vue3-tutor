<template>
  <section>
    ...
    <el-button @click="changeImg">改变image</el-button>
    <section class="py-4 text-center date-selector">
      ...
      <button @click="incrementDay(currentDate)" :disabled="currentDate >= today">Next Day</button>
    </section>
    ...
    <template v-else-if="image">
      ...
      <figure class="mb-0">
        <iframe
          ...
          allowfullscreen
        ></iframe>
        <img v-else class="max-w-full m-auto max-h-[75vh]" :src="image.url" :alt="image.title" />
...
import { luna } from "@/constants/imgs";

const nasa = useNasaPOD();

const today = new Date().toISOString().slice(0, 10);
function changeImg() {
  currentDate.value = luna;
}
