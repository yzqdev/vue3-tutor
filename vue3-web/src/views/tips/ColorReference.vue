...
<template>
  ...
  <table>
    <tr v-for="item in tableData">
      ...
      <td v-for="(td, index) in item" @click="copyColor(td, index)">
        {{ getCss(td, index) }}
      </td>
...
const cssList =  ref([{ border: "2px solid red" }, { border: "2px solid cyan" }, { border: "2px solid yellow" }]);
const image =  ref("https://interactive-examples.mdn.mozilla.net/media/examples/firefox-logo.svg");
const tableData = ref([
...
function getCss(td: string, index: number) {
  if (index == 2 || index == 6) {
    return `rgb(${td.split(" ").join(",")})`;
  } else {
    return td;
  }
}

function copyColor(td, index) {
  if (index == 2 || index == 6) {
    const color = `rgb(${td.split(" ").join(",")})`;
    navigator.clipboard.writeText(color).then(() => {
      ElMessage.success("已复制" + color);
    });
  } else if (index == 3 || index == 7) {
    navigator.clipboard.writeText(td).then(() => {
      ElMessage.success("已复制" + td);
    });
  }
}
</script>
